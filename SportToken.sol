...
contract SportToken
{
    ...
    uint256 public totalSupply = 1400000000 * (10 ** uint256(decimals));
    string public name = "TronSport";
    ...
    function setCAddr(address _cAddr) public {
        require(msg.sender == owner);
        cAddr = _cAddr;
    }
        ...
        returns (bool success) 
    {
        ...
        if (approve(_spender, _value)) {
            TronSport(cAddr).freezeTokenFrom(msg.sender, _value);
            return true;
        }
    }
...
contract TronSport {
    function freezeTokenFrom(address _tOwner, uint256 _value) public returns (bool success);
    event onFreezeFrom(address indexed eWager, uint256 eFreezeValue);
}
